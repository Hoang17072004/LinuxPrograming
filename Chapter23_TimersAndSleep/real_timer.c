...
//the current value and interval for ITIMER_REAL timer
static void displayTimers(const char *msg,Boolean includeTime) {
	struct itimerval itv;
	static struct timeval start;
	struct timeval curr;
	static int callNum=0;//Number of calls to this function
	if (callNum==0) if (gettimeofday(&start,NULL)==-1) errExit("gettimeofday");
	if (callNum%20==0) {
...
int main(int argc,char *argv[]) {
	struct itimerval itv;
	clock_t prevClock;
	int maxSigs;//Number of signals to catch before existing
	...
	//Exit after 3 signals , or on first signal if intervals 0
	maxSigs=(itv.it_interval.tv_sec==0&&itv.it_interval.tv_usec==0)?1:3;
	displayTimers("START: ",FALSE);
