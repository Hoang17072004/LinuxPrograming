...
#include "../Lib/itimerspec_from_str.h" //Declares itimerspecFromStr()
#include<stdio.h>
...
static void threadFunc(union sigval sv){
	...
	/*Increment counter varable shared with main thread and signal
...
int main(int argc,char *argv[]) {
	...
	//Could be pointer to thread_attr_t structure
	...
	on each invocation of thread notification function. We print a message
	so that the user can see that this occurred*/
	s=pthread_mutex_lock(&mtx);
	if (s!=0) errExitEN(s,"pthread-mutex_lock");
	for (;;) {
