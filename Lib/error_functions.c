...
#include <stdio.h>      // Cho vsnprintf, fprintf, fflush, stdout, stderr...
#include <stdlib.h>     // Cho getenv, exit, abort
#include <string.h>     // Cho strerror
#include <unistd.h>     // Cho _exit
#include <errno.h>      // Cho errno
#include <stdarg.h>     // Cho va_list (bạn đã có dòng này)
...
static void terminate(Boolean useExit3) {
	char *s;
	//Dump core if EF_DUMPCORE environment variable is define
...
static void outputError(Boolean useErr,int err,Boolean flushStdout,const char *format,va_list ap) {
	...
	fputs(buf,stderr);
	fflush(stderr);//In case stderr is not line-buffer
}
