...
static void displayProcessTimes(const char *msg) {
	...
	if (clockTicks==0) {
		clockTicks=sysconf(_SC_CLK_TCK);//Fetch clock ticks on first call
		if (clockTicks==-1) errExit("sysconf");
	}
	clockTime=clock();
	if (clockTicks==-1) errExit("clock");
	printf("	clock() returns: %ld clocks-per-sec (%.2f secs)\n",
	(long)clockTime,(double)clockTime/CLOCKS_PER_SEC);
	if (times(&t)==-1) errExit("times");
...
int main(int argc,char *argv[]) {
	...
	for (j=0;j<numCalls;j++) (void)getppid();
