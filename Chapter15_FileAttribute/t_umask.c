...
int main(int argc,char *argv[]) {
	...
	mode_t u;
	umask(UMASK_SETTING);
	fd=open(MYFILE,O_RDWR|O_CREAT,FILE_PERMS);
	if (fd==-1) errExit("open-%s",MYDIR);
	if (mkdir(MYDIR,DIR_PERMS)==-1) errExit("dir");
	u=umask(0);//Retriesves (and clears) umask value

	if (stat(MYFILE,&sb)==-1) errExit("stat-%s",MYFILE);

	printf("Requested file perms: %s\n",filePermStr(FILE_PERMS,0));
	printf("Process umask: %s\n",filePermStr(u,0));
	printf("Actual file perm: %s\n",filePermStr(sb.st_mode,0));
