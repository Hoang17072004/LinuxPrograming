...
static void handler(int sig) {
	/*UNSAGE: This handler uses non-async-signal-safe functions
...
int main(int argc,char *argv[]) {
	...
	if (sigaction(SIGINT,&sa,NULL)==-1) errExit("sigation");
#ifdef USE_SIGSETJMP
	...
	else printSigMask(stdout,"After jump from handler, signal mask is: \n");
	for (;;) pause();
}
